...
[lev,lat,lon,time,air]=opennc([loadfilepath_P,'air/air.',num2str(yyyy),'.casesubset.nc'],'air');air=air*0.01+465.15;
[lev,lat,lon,time,hgt]=opennc([loadfilepath_P,'hgt/hgt.',num2str(yyyy),'.casesubset.nc'],'hgt');hgt=hgt+31265;
[lev,lat,lon,time,uwnd]=opennc([loadfilepath_P,'uwnd/uwnd.',num2str(yyyy),'.casesubset.nc'],'uwnd');uwnd=uwnd*0.01+187.65;
[lev,lat,lon,time,vwnd]=opennc([loadfilepath_P,'vwnd/vwnd.',num2str(yyyy),'.casesubset.nc'],'vwnd');vwnd=vwnd*0.01+187.65;
[lev,lat,lon,time,rhum]=opennc([loadfilepath_P,'rhum/rhum.',num2str(yyyy),'.casesubset.nc'],'rhum');rhum=rhum*0.01+302.65;
[lev,lat,lon,time,omega]=opennc([loadfilepath_P,'omega/omega.',num2str(yyyy),'.casesubset.nc'],'omega');omega=omega*0.001+28.765;
nlev=length(lev);
...
[isk,lat,lon,time,uwndk]=opennc([loadfilepath_K,'uwndk',num2str(yyyy),'.nc'],'uwndk');
 [isk,lat,lon,time,vwndk]=opennc([loadfilepath_K,'vwndk',num2str(yyyy),'.nc'],'vwndk');
[pv,latd,lond,iskd]=compute_pv(yyyy,uwndk,vwndk,lat,lon,isk,time,6);
...
[sym_stab,latdd,londd,iskd]=compute_symmetricstab(yyyy,uwnd,vwnd,hgt,lat,lon,lev,time,6);
...
[DVA,LOTA,latddd,londdd,levd,fsigma]=compute_omegaterms(uwnd,vwnd,air,hgt,levm,lat,lon,lev,time,45);
...
for tt=1:ntime
#  for tt=3
  [qg]=compute_qgw(squeeze(DVA(:,lt1:lt2,ln1:ln2,tt)),squeeze(LOTA(:,lt1:lt2,ln1:ln2,tt)),squeeze(omega(:,ltw1-1:ltw2+1,lnw1-1:lnw2+1,tt)),squeeze(fsigma(:,lt1:lt2,ln1:ln2,tt)),latddd,londdd,lev);
...
[isk,lat,lon,time,hgtk]=opennc([loadfilepath_K,'hgtk',num2str(yyyy),'.nc'],'hgtk');
 [hgtk_roc,hgtk_roc_daily,timed,timed_daily]=compute_dvar_dt(hgtk,time);
  writenc(yyyy,hgtk_roc,lat,lon,isk,timed,"isk","hgtk_roc",savefilepath);
   writenc(yyyy,hgtk_roc_daily,lat,lon,isk,timed_daily,"isk","hgtk_roc_daily",savefilepath);
...
[hrs]=hrssince('20071216',09,'ncep2');
R=DVA(find(levd==250),:,:,find(time==hrs));
