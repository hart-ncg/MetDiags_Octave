...
# in isentropic system latent heating rate (LHR) can be calculated
dpot_Te=(pot_Te(3:end,:,:,:)-pot_Te(1:end-2,:,:,:));
dpot_T=(pot_T(3:end,:,:,:)-pot_T(1:end-2,:,:,:));
dp=P(3:end,:,:,:)-P(1:end-2,:,:,:);dp=dp.*100;
...
# shear vorticity
[du_dp,levd]=compute_dp(u,lev);du_dp=du_dp(2:end-1,2:end-1,2:end-1,:)./dp(2:end-1,2:end-1,2:end-1,:);du_dp=du_dp.*(-1);
[dv_dp,levd]=compute_dp(v,lev);dv_dp=dv_dp(2:end-1,2:end-1,2:end-1,:)./dp(2:end-1,2:end-1,2:end-1,:);
######### OR
...
[xgrad,ygrad,zgrad,latd,lond,levdd]=compute_3Dgradient(H,lat,lon,levd);zgrad=zgrad.*(-1);
